#!/usr/bin/env python3
##############################################################################
# SCRIPT NUTANIX TOOLS FOR AHV v1.7.5                                        #
# PYTHON 3.6                                                                 #
...
version_soft = 'Nutanix tools for AHV v1.7.5'
print(version_soft)
start = time.time()
...
if pe_success_login == 100:
    ...
    worksheet0.write('A2', 'CLUSTER AOS VERSION', bold)
    worksheet0.write('A3', 'CLUSTER AHV VERSION', bold)
    worksheet0.write('A4', 'CLUSTER NCC VERSION', bold)
    worksheet0.write('A5', 'CLUSTER SUBNET', bold)
    worksheet0.write('A6', 'CLUSTER TIMEZONE', bold)
    worksheet0.write('A7', 'CLUSTER DNS', bold)
    worksheet0.write('A8', 'CLUSTER NTP', bold)
    worksheet0.write('A9', 'CLUSTER N°NODES', bold)
    worksheet0.write('A10', 'CLUSTER STORAGE TYPE', bold)
    worksheet0.write('A11', 'CLUSTER IP', bold)
    worksheet0.write('A12', 'CLUSTER DATA SERVICE IP', bold)
    worksheet0.write('A13', 'CLUSTER REDUNDANCY FACTOR', bold)
    worksheet0.write('A14', 'TOTAL VM', bold)
    worksheet0.write('A15', 'HA ENABLED', bold)
    worksheet0.write('A16', 'HA RESERVATION', bold)
    worksheet0.write('A17', 'HA HOST TOLERATE', bold)
    worksheet0.write('A18', 'HA STATE', bold)
    worksheet0.write('A19', 'RESILIENCY STATUS LEVEL', bold)
    worksheet0.write('A20', 'N°DISK CAN BE FAIL ON METADATA', bold)
    worksheet0.write('A21', 'DISK IS REBUILDING ', bold)
    worksheet0.write('A22', 'N°DISK CAN BE FAIL ON ERASURE_CODE_STRIP_SIZE', bold)
    worksheet0.write('A23', 'DISK IS REBUILDING ', bold)
    worksheet0.write('A24', 'N°DISK CAN BE FAIL ON EXTENT_GROUPS', bold)
    worksheet0.write('A25', 'DISK IS REBUILDING ', bold)
    worksheet0.write('A26', 'N°DISK CAN BE FAIL ON OPLOG', bold)
    worksheet0.write('A27', 'DISK IS REBUILDING ', bold)
    worksheet0.write('A28', 'DATE', bold)
    ...
    def vminfodef():
        ...
        for vm in entities:  ### VM WITH NIC
            for nic in vm[4]:
                ...
                col = 0
                worksheet1.write(row, col + 3, cluster_name)
                    ...
                    for project in pc_vm_proyect_raw['entities']:
                        ...
                        if project['spec']['name'] == vm[0]:
                            ...
                            if not 'guest_tools' in project['spec']['resources'].keys():
                                worksheet1.write(row, col + 20, 'Enable for check')
                                worksheet1.write(row, col + 21, '-')
                                worksheet1.write(row, col + 22, '-')
                                worksheet1.write(row, col + 23, '-')
                            # worksheet1.write(row , col +24 ,'-')
            ...
            if disk['is_cdrom'] == True:
                if info['disk_label'] == 'ide.0':
                    ...
                    if disk['is_empty'] == True:
                        worksheet2.write(row, col + 14, 'NO ISO MOUNTED')
                    else:
                        worksheet2.write(row, col + 14, 'ISO MOUNTED')
                else:
    ...
    for host in ahvhost:
        ...
        worksheet5.write(row, col + 18, host[18])
        worksheet5.write(row, col + 19, host[19])
    ...
    for ahvdisk_raw in ahvdisk:
        ...
        worksheet10.write(row, col, host)
        for x in ahvhost:
            if x[2] == host:
                worksheet10.write(row, col + 1, x[1])  ## print hostname
        worksheet10.write(row, col + 2, cvmip)
        worksheet10.write(row, col + 3, tier)
        worksheet10.write(row, col + 4, location)
        worksheet10.write(row, col + 5, '=' + (str(disksize)) + '/1024/1024/1024')
        worksheet10.write(row, col + 6, diskonline)
        worksheet10.write(row, col + 7, diskstatus)
        worksheet10.write(row, col + 8, diskmodel)
        worksheet10.write(row, col + 9, diskfirmware)
        worksheet10.write(row, col + 10, diskserial)
        worksheet10.write(row, col + 11, diskisbad)
