...
version_soft = 'Nutanix tools for AHV v1.0' 
print(version_soft)
...
if  (pe_vm_info.status_code == requests.codes.ok
	...
	print('Login Succesfully API V2 on Prism Element')
	time.sleep(2)
else:
	print('Login Failed to API V2, Bad Password , Bad IP/FQDN Prism Element or networks problems. Run again the script')
	sys.exit()
...
if  pe_success_login == 100: 
    ...
    if pe_vm_info.status_code == requests.codes.ok:    
        ...
        entities = [(x['name'], x['uuid'], x['power_state'], x.get('host_uuid', '-'), x['vm_nics'], x['num_cores_per_vcpu'], x['num_vcpus'], x['timezone'], x['vm_disk_info'],x['memory_mb']) for x in out_json['entities']]
    ...
    worksheet0.write('A16', 'DATE', bold)
    ...
    if confirm == 'y':
        worksheet1.write('S1', 'Project', bold)
    ...
    worksheet5.write('D1', 'CVM IP', bold)
    worksheet5.write('E1', 'SERIAL NODE', bold)
    ...
    worksheet0.write(row+15 , 1 ,today.strftime('Running on %d, %b %Y'), boldcell)
    ...
    for vm in entities:
    	for nic in vm[4]:
            ...
            if confirm == 'y':
            	worksheet1.write(row , col +18 ,pc_vm_single_project)
    ...
    for vgroup in vg:
        for volume in vgroup[2]:
            ...
            #for grpoup in vgroup[3]:          
            #    vmname= "="+"VLOOKUP"+"(E"+(str(raw))+",'VM INFO'!E:F,COLUMN('VM INFO'!F:F)-COLUMN('VM INFO'!E:F)+1,0)"
            #    worksheet6.write(row , col +4 ,grpoup.get('vm_uuid', '-'))
            #    worksheet6.write(row , col +5 ,vmname)
    
            #else :
            #   worksheet6.write(row , col +4 ,'-')
            #   worksheet6.write(row , col +5 ,'-')
    ...
    for image_not_size in image:
        if not 'vm_disk_size' in image_not_size.keys():
            ...
            #print('NAME_1: ',nameimage)   
            #print('IMAGE STATE: ',image_type)
            #print('IMAGE STATE: ',image_state)
            #print('IMAGE STATE: ',created_time_in_usecs)         
