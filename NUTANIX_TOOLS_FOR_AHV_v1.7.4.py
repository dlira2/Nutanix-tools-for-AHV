#!/usr/bin/env python3
##############################################################################
# SCRIPT NUTANIX TOOLS FOR AHV v1.7.4                                        #
# PYTHON 3.6                                                                 #
...
version_soft = 'Nutanix tools for AHV v1.7.4'
print(version_soft)
start = time.time()
...
try:
	...
	pe_ahvdisk = requests.get(url + '/PrismGateway/services/rest/v2.0/disks/', auth=(username, password), verify=False, timeout=TIMEOUT)
					...
					for project in pc_vm_proyect_raw['entities']:
						...
						if project['spec']['name'] == vm[0]:
							if 'guest_tools' in project['spec']['resources'].keys():
								if 'ngt_state' in project['spec']['resources']['guest_tools']['nutanix_guest_tools'].keys():
									pc_vm_guest_install= project['spec']['resources']['guest_tools']['nutanix_guest_tools']['ngt_state']
								...
								if 'state' in project['spec']['resources']['guest_tools']['nutanix_guest_tools'].keys():
									pc_vm_guest_state= project['spec']['resources']['guest_tools']['nutanix_guest_tools']['state']
									worksheet1.write(row , col +20 ,pc_vm_guest_state)
									#print('state:',pc_vm_guest_install)
								else:
									worksheet1.write(row , col +20 ,'-')
								if 'is_reachable' in project['spec']['resources']['guest_tools']['nutanix_guest_tools'].keys():
									pc_vm_guest_reachable= project['spec']['resources']['guest_tools']['nutanix_guest_tools']['is_reachable']
									worksheet1.write(row , col +23 ,pc_vm_guest_reachable)
									#print('is_reachable:',pc_vm_guest_install)
								else:
									worksheet1.write(row , col +23 ,'-')
								if 'version' in project['spec']['resources']['guest_tools']['nutanix_guest_tools'].keys():
									pc_vm_guest_version=  project['spec']['resources']['guest_tools']['nutanix_guest_tools']['version']
									worksheet1.write(row , col +22 ,pc_vm_guest_version)
									#print('version:',pc_vm_guest_install)
								else:
									worksheet1.write(row , col +22 ,'-')
								if 'guest_os_version' in project['spec']['resources']['guest_tools']['nutanix_guest_tools'].keys():
									pc_vm_guest_os_version= project['spec']['resources']['guest_tools']['nutanix_guest_tools']['guest_os_version']
